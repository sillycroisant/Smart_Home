...
# CONFIG
# =========================
VIDEO_PATH = "./video/APhu/inside.mp4"
DB_DIR = "./db"
UNKNOWN_DIR = "./unknown"
    ...
    for track in tracks:
        ...
        if tid not in track_final:
            ...
            if len(faces) == 1:
                ...
                if fx2 > fx1 and (fx2 - fx1) >= FACE_MIN_SIZE:
                    emb = face.embedding
                    emb = emb / np.linalg.norm(emb)
            ...
            if len(track_votes[tid]) >= VOTE_LEN:
                ...
                if final == "UNKNOWN":
                    unknown_count += 1
                    final_id = f"UNKNOWN_{unknown_count:03d}"

                    cv2.imwrite(
                        f"{UNKNOWN_DIR}/{final_id}.jpg",
                        person_roi
                    )
                else:
                    final_id = final
...
# =========================
    ...
    cv2.imshow("FIXED PIPELINE", frame)
